...
def Logo():
    ...
    ╚═══════════════════════════════════════════════════════╝
    {white}═══════════════════════════════════════════════════════════
 {BEFORE}01{AFTER} Username Scanner      {blue}(Auto Check Multiple Sites){white}
 {BEFORE}02{AFTER} Advanced Tracker      {blue}(Manual Search & Research){white}
 {BEFORE}03{AFTER} Dox Creator           {blue}(Complete Dossier Generator){white}
 {BEFORE}04{AFTER} Email Services        {blue}(Email Tracker & Lookup){white}
 {BEFORE}05{AFTER} Phone Lookup          {blue}(Phone Number Information){white}
...
def username_scanner():
    Clear()
    print(f"{red}╔═══════════════════════════════════════╗{white}")
    print(f"{red}║      AUTO USERNAME SCANNER           ║{white}")
    print(f"{red}╚═══════════════════════════════════════╝{white}\n")
    
    user = input(f"{BEFORE + current_time_hour() + AFTER} {INPUT} Username -> {reset}")
    ...
    sites_list = {
        ...
        "DeviantArt": f"https://{user}.deviantart.com"
    }
    
    print(f"\n{BEFORE + current_time_hour() + AFTER} {WAIT} Scanning {white}{len(sites_list)}{red} platforms...{reset}\n")
    time.sleep(1)
    
    found_count = 0
    
    for platform, url in sites_list.items():
        try:
            response = requests.get(url, timeout=3, allow_redirects=True)
            if response.status_code == 200:
                print(f"{green}[✓ FOUND]{white} {platform:15} → {blue}{url}{reset}")
                found_count += 1
            else:
                print(f"{red}[✗ NOT FOUND]{white} {platform}{reset}")
        except requests.exceptions.RequestException:
            print(f"{yellow}[? TIMEOUT]{white} {platform}{reset}")
        
        time.sleep(0.3)
    
    print(f"\n{BEFORE + current_time_hour() + AFTER} {SUCCESS} Scan complete! Found on {white}{found_count}/{len(sites_list)}{green} platforms{reset}")
...
def advanced_tracker():
    Clear()
    print(f"{red}╔═══════════════════════════════════════╗{white}")
    print(f"{red}║       ADVANCED TRACKER TOOL          ║{white}")
    print(f"{red}╚═══════════════════════════════════════╝{white}\n")
    
    print(f" {BEFORE}01{AFTER} Username Search")
    print(f" {BEFORE}02{AFTER} Name Search (FirstName + LastName)")
    print(f" {BEFORE}03{AFTER} Custom Search")
    ...
    if search_type in ['01', '1']:
        query = input(f"{BEFORE + current_time_hour() + AFTER} {INPUT} Username -> {reset}")
        search_query = query
        ...
        search_query = input(f"{BEFORE + current_time_hour() + AFTER} {INPUT} Custom Query -> {reset}")
    else:
    ...
    if not search_query.strip():
        print(f"{red}[ERROR] Search query cannot be empty{reset}")
        time.sleep(2)
        return
    ...
    Clear()
    print(f"{red}╔═══════════════════════════════════════╗{white}")
    print(f"{red}║     SELECT SEARCH PLATFORMS          ║{white}")
    print(f"{red}╚═══════════════════════════════════════╝{white}\n")
    print(f" {BEFORE}01{AFTER} Facebook")
    print(f" {BEFORE}02{AFTER} YouTube")
    print(f" {BEFORE}03{AFTER} Twitter/X")
    print(f" {BEFORE}04{AFTER} TikTok")
    print(f" {BEFORE}05{AFTER} PeekYou")
    print(f" {BEFORE}06{AFTER} Tumblr")
    print(f" {BEFORE}07{AFTER} Google Search")
    print(f" {BEFORE}08{AFTER} LinkedIn")
    print(f" {BEFORE}09{AFTER} Instagram")
    print(f" {BEFORE}99{AFTER} ALL PLATFORMS")
    print(f" {BEFORE}00{AFTER} Back\n")
    
    platforms = {
        '1': ('Facebook', f"https://www.facebook.com/search/top/?q={search_query}"),
        '2': ('YouTube', f"https://www.youtube.com/results?search_query={search_query}"),
        '3': ('Twitter/X', f"https://twitter.com/search?q={search_query}"),
        '4': ('TikTok', f"https://www.tiktok.com/search?q={search_query}"),
        '5': ('PeekYou', f"https://www.peekyou.com/{search_query.replace(' ', '_')}"),
        '6': ('Tumblr', f"https://www.tumblr.com/search/{search_query}"),
        '7': ('Google', f"https://www.google.com/search?q={search_query}"),
        '8': ('LinkedIn', f"https://www.linkedin.com/search/results/all/?keywords={search_query}"),
        '9': ('Instagram', f"https://www.instagram.com/explore/tags/{search_query.replace(' ', '')}/")
    }
    
    while True:
        choice = input(f"{BEFORE + current_time_hour() + AFTER} {INPUT} Platform (00 to exit) -> {reset}")
        
        if choice in ['0', '00']:
            break
        elif choice == '99':
            print(f"\n{BEFORE + current_time_hour() + AFTER} {WAIT} Opening all platforms...{reset}")
            for name, url in platforms.values():
                print(f"{green}[+]{white} Opening {name}...")
                webbrowser.open(url)
                time.sleep(0.5)
            break
...
def email_services():
    ...
    print(f" {BEFORE}01{AFTER} Email Tracker (Social Scan)")
    print(f" {BEFORE}02{AFTER} Email Lookup (DNS/MX)")
    print(f" {BEFORE}00{AFTER} Back\n")
...
def phone_lookup():
    ...
    print(f"{yellow}[INFO] Enter phone number with country code (e.g., +33123456789){reset}\n")
    ...
    try:
        ...
        print(f"{yellow}[INFO] Use format: +[country_code][number]{reset}")
